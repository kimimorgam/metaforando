...
def analyze_frame(cap, model, analyze_fps, video_fps):
    ...
    progress_bar["maximum"] = frame_total
    ...
    while cap.isOpened():
        ...
        if frame_count % (video_fps // int(analyze_fps)) == 0:
            ...
            for (x, y, w, h) in faces:
                ...
                progress_bar["value"] = cap.get(cv2.CAP_PROP_POS_FRAMES)

                # Atualiza o tempo estimado
                time_elapsed = frame_count / video_fps
                time_remaining = (cap.get(cv2.CAP_PROP_FRAME_COUNT) - frame_count) / video_fps
                time_label.config(
                    text=f"Tempo decorrido: {time_elapsed:.2f} segundos\nTempo estimado restante: {time_remaining:.2f} segundos")
...
time_label = tk.Label(root, text="Tempo decorrido: 0.00 segundos\nTempo estimado restante: 0.00 segundos")
time_label.pack()
